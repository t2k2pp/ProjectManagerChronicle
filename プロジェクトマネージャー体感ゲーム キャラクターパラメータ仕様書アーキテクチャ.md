...
|**`stats_blue`**|object|**表スキル（プロジェクト遂行）**<br><br>  <br><br>{ design, develop, test, negotiation, idea, judgment } (各0-10)|
|**`stats_red`**|object|**裏スキル（チーム運営）**<br><br>  <br><br>{ admin, organizer, service, chat, charm, luck } (各0-10)|
|`stamina`|object|{ current: number, max: number, recovery_rate: number }|
|`traits`|array|特性タグ ["vacation_lover", "night_owl", "awakened"]|
