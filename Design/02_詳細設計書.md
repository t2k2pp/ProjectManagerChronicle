...
| ageType | enum | 体力特性（BURST/ENDURANCE/EFFICIENT） | 年齢ではなく働き方タイプで特徴付け |
...
| ageType | 説明 | max値 | 回復力 | 消費効率 |
|---------|------|-------|--------|----------|
| BURST | 瞬発型（若手傾向） | 120 | 高（15/日） | 標準 |
| ENDURANCE | 持続型（中堅傾向） | 100 | 中（10/日） | 良好（0.9倍） |
| EFFICIENT | 効率型（ベテラン傾向） | 80 | 低（8/日） | 最良（0.8倍） |

**設計判断の根拠**:
- **ダブル・ヘキサゴン**: 実務能力と対人能力を分離し、「技術は高いが協調性がない」などのリアルな人物像を表現
- **ageType enum**: 年齢を直接持たず、働き方の特性で管理。年齢差別を避けつつゲーム性を確保
...
| 特性 | 希望休日数 | 金銭交渉 | AP交渉 | 強制変更 | 備考 |
|------|-----------|----------|--------|----------|------|
| vacation_lover | 7日 | ❌ 不可 | ❌ 不可 | ❌ 不可 | ゲームオーバーリスク |
| plan_oriented | 5日 | ❌ 不可 | ⚠️ AP×3 | ⚠️ 士気-30 | 事前計画重視 |
| money_lover | 3日 | ✅ 5万円/日 | ❌ 不可 | ❌ 不可 | 報酬で説得 |
| standard | 2日 | ✅ 3万円/日 | ✅ AP×1 | ⚠️ 士気-10 | 柔軟対応 |

**交渉コスト詳細**:

| 交渉方法 | コスト | 成功率 | 失敗時ペナルティ |
|---------|--------|--------|------------------|
| 金銭交渉 | 上記/日 | 100% | なし |
| AP交渉 | AP×係数 | 100% | なし |
| 強制変更 | なし | 100% | 士気低下（上記） |
...
- **交渉手段の多様性**: 予算/AP/士気のトレードオフを学習要素として組み込み
