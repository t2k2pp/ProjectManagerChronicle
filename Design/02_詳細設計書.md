# PM立志伝：プロジェクト・クロニクル 詳細設計書

**Version**: 1.0  
**Last Updated**: 2025-12-30  
**Document Type**: Detailed Design Specification

---

## 1. プロジェクト構成

### 1.1 ディレクトリ構造

```
pm-chronicle/
├── public/                      # 静的アセット
│   ├── images/                 # 画像素材
│   │   ├── characters/        # キャラクター画像
│   │   ├── ui/                # UIアイコン
│   │   └── backgrounds/       # 背景画像
│   ├── sounds/                # 効果音・BGM
│   └── manifest.json          # PWA設定
│
├── src/
│   ├── main.tsx               # エントリーポイント
│   ├── App.tsx                # ルートコンポーネント
│   │
│   ├── components/            # UIコンポーネント
│   │   ├── common/           # 共通コンポーネント
│   │   │   ├── Button.tsx
│   │   │   ├── Modal.tsx
│   │   │   ├── Card.tsx
│   │   │   └── HexagonChart.tsx
│   │   │
│   │   ├── game/             # ゲーム固有コンポーネント
│   │   │   ├── PMCockpit/   # メイン画面
│   │   │   │   ├── OfficeView.tsx
│   │   │   │   ├── GanttChart.tsx
│   │   │   │   ├── EVMeter.tsx
│   │   │   │   └── CalendarDisplay.tsx
│   │   │   │
│   │   │   ├── CardBattle/   # カードバトル
│   │   │   │   ├── BattleField.tsx
│   │   │   │   ├── CardHand.tsx
│   │   │   │   ├── DirectNegotiation.tsx
│   │   │   │   └── GaugeDisplay.tsx
│   │   │   │
│   │   │   ├── VacationPuzzle/  # 休暇調整パズル
│   │   │   │   ├── CalendarGrid.tsx
│   │   │   │   ├── MemberBlock.tsx
│   │   │   │   └── NegotiationDialog.tsx
│   │   │   │
│   │   │   └── CharacterSheet/  # キャラクター詳細
│   │   │       ├── StatusDisplay.tsx
│   │   │       └── TrueNameReveal.tsx
│   │   │
│   │   └── screens/          # 画面コンポーネント
│   │       ├── TitleScreen.tsx
│   │       ├── GameSetup.tsx
│   │       ├── Dashboard.tsx
│   │       ├── ProjectSelect.tsx
│   │       └── ReportScreen.tsx
│   │
│   ├── hooks/                # カスタムフック
│   │   ├── useGameState.ts
│   │   ├── useProject.ts
│   │   ├── useCharacter.ts
│   │   ├── useWeekTurn.ts
│   │   └── useIndexedDB.ts
│   │
│   ├── lib/                  # ビジネスロジック
│   │   ├── engine/          # ゲームエンジン
│   │   │   ├── turnProcessor.ts     # ターン処理
│   │   │   ├── evmCalculator.ts     # EVM計算
│   │   │   ├── eventGenerator.ts    # イベント生成
│   │   │   ├── troubleResolver.ts   # トラブル解決
│   │   │   └── aiJudge.ts          # AI判定（直談判）
│   │   │
│   │   ├── generators/      # データ生成
│   │   │   ├── worldGenerator.ts    # 世界環境生成
│   │   │   ├── characterGenerator.ts
│   │   │   ├── projectGenerator.ts
│   │   │   └── rivalGenerator.ts
│   │   │
│   │   ├── calculators/     # 各種計算ロジック
│   │   │   ├── staminaCalculator.ts
│   │   │   ├── progressCalculator.ts
│   │   │   ├── qualityCalculator.ts
│   │   │   └── riskCalculator.ts
│   │   │
│   │   └── validators/      # バリデーション
│   │       ├── assignmentValidator.ts
│   │       └── budgetValidator.ts
│   │
│   ├── store/               # 状態管理
│   │   ├── gameStore.ts     # Zustandストア（全体）
│   │   ├── slices/          # ストアスライス
│   │   │   ├── worldSlice.ts
│   │   │   ├── projectSlice.ts
│   │   │   ├── characterSlice.ts
│   │   │   └── uiSlice.ts
│   │   └── selectors.ts     # セレクター
│   │
│   ├── db/                  # データベース
│   │   ├── schema.ts        # Dexie.jsスキーマ定義
│   │   ├── migrations.ts    # マイグレーション
│   │   └── repositories/    # リポジトリパターン
│   │       ├── characterRepository.ts
│   │       ├── projectRepository.ts
│   │       └── logRepository.ts
│   │
│   ├── services/            # 外部連携サービス
│   │   ├── aiService.ts     # AI API通信
│   │   ├── exportService.ts # データエクスポート
│   │   └── importService.ts # データインポート
│   │
│   ├── types/               # 型定義
│   │   ├── character.ts
│   │   ├── project.ts
│   │   ├── task.ts
│   │   ├── event.ts
│   │   └── world.ts
│   │
│   ├── constants/           # 定数
│   │   ├── gameConstants.ts
│   │   ├── cardDefinitions.ts
│   │   ├── eventDefinitions.ts
│   │   └── npcDefinitions.ts
│   │
│   ├── utils/               # ユーティリティ
│   │   ├── dateUtils.ts
│   │   ├── randomUtils.ts
│   │   ├── formatUtils.ts
│   │   └── cryptoUtils.ts   # チェックサム生成
│   │
│   └── styles/              # スタイル
│       ├── globals.css
│       └── theme.ts         # Tailwind設定拡張
│
├── tests/                   # テスト
│   ├── unit/
│   ├── integration/
│   └── e2e/
│
├── package.json
├── tsconfig.json
├── vite.config.ts
├── tailwind.config.js
└── README.md
```

---

## 2. データベース詳細設計

### 2.1 IndexedDB スキーマ定義（Dexie.js）

**ファイル**: `src/db/schema.ts`

```typescript
import Dexie, { Table } from 'dexie';

// 型定義
export interface SaveSlot {
  id: number;                    // 1-5（スロット番号）
  name: string;                  // セーブ名
  createdAt: Date;
  updatedAt: Date;
  worldState: WorldState;
}

export interface WorldState {
  version: string;               // "1.0"
  seed: number;                  // ランダムシード値
  calendar: CalendarState;
  company: CompanyProfile;
  characters: Character[];
  rivals: RivalCompany[];
  partners: BusinessPartner[];
  projects: Project[];
  currentProjectId: string | null;
}

export interface CalendarState {
  currentYear: number;           // 2025
  currentMonth: number;          // 1-12（4月始まり）
  currentWeek: number;           // 累積週数
  seasonEvents: SeasonEvent[];   // 年間イベント
}

export interface Character {
  id: string;
  name: string;
  ageGroup: '20s' | '30-40s' | '50s+';
  gender: 'male' | 'female' | 'other';
  jobClass: string;
  isUnique: boolean;             // 固有NPC判定
  
  // ダブル・ヘキサゴン
  statsBlue: ProjectSkills;
  statsRed: TeamSkills;
  
  // 体力システム
  stamina: StaminaState;
  
  // 特性タグ
  traits: string[];
  
  // 信頼度・覚醒
  trustLevel: number;            // 0-100
  isAwakened: boolean;
  
  // 隠しデータ（覚醒後開放）
  hiddenData?: {
    trueName: string;
    trueStatsBlue?: Partial<ProjectSkills>;
    trueStatsRed?: Partial<TeamSkills>;
    secretTrait?: string;
  };
  
  // 現在の状態
  currentAssignment?: string;    // プロジェクトID
  mood: number;                  // 士気（0-100）
  vacationPlan?: VacationPlan;
}

export interface ProjectSkills {
  design: number;      // 0-10
  develop: number;
  test: number;
  negotiation: number;
  propose: number;
  judgment: number;
}

export interface TeamSkills {
  admin: number;
  organizer: number;
  service: number;
  chat: number;
  charm: number;
  luck: number;
}

export interface StaminaState {
  current: number;
  max: number;
  recoveryRate: number;
  exhaustionDebuff: number;      // 蓄積疲労デバフ
}

export interface Project {
  id: string;
  name: string;
  clientType: 'existing' | 'new';
  clientCategory: 'private' | 'public';
  domain: string;                // '金融', '医療', '一般'
  
  budget: BudgetState;
  schedule: ScheduleState;
  
  // QCD指標
  qualityScore: number;
  deliveryScore: number;
  costScore: number;
  
  // EVM
  evm: EVMState;
  
  // タスク
  wbs: Task[];
  
  // リスク管理
  riskRegistry: Risk[];
  
  status: 'PLANNING' | 'RUNNING' | 'COMPLETED' | 'FAILED';
}

export interface Task {
  id: string;
  projectId: string;
  name: string;
  phase: 'REQUIREMENT' | 'DESIGN' | 'DEVELOP' | 'TEST' | 'DEPLOY';
  
  assigneeId: string | null;
  
  // 進捗
  progress: number;              // 0-100
  estimatedEffort: number;       // 人日
  actualEffort: number;
  
  // 品質
  qualityScore: number;          // 0-100
  bugCount: number;
  
  // リスク
  riskFactor: number;            // 0-100（高いほど危険）
  isRiskVisible: boolean;        // 判断スキルで可視化
  
  // 依存関係
  dependencies: string[];        // 先行タスクID配列
  isCriticalPath: boolean;
  
  // ステータス
  isBlocked: boolean;
  blockerReason?: string;
}

export interface WeeklyLog {
  id: string;
  projectId: string;
  week: number;
  
  eventType: 'PROGRESS' | 'TROUBLE' | 'DECISION' | 'NEGOTIATION' | 'VACATION';
  severity: 'info' | 'warning' | 'critical';
  
  title: string;
  description: string;
  
  // AI分析用
  involvedCharacters: string[];  // キャラクターID配列
  cardsUsed?: string[];          // 使用カード名
  pmChoice?: string;             // プレイヤーの選択肢
  pmFreeText?: string;           // 直談判時の入力
  
  // 結果
  impact: {
    budgetChange?: number;
    scheduleChange?: number;
    qualityChange?: number;
    moralChange?: number;
  };
  
  timestamp: Date;
}

// Dexieデータベース定義
export class PMChronicleDB extends Dexie {
  saves!: Table<SaveSlot, number>;
  logs!: Table<WeeklyLog, string>;

  constructor() {
    super('pm_chronicle_db');
    
    this.version(1).stores({
      saves: 'id, updatedAt',
      logs: 'id, projectId, week, eventType',
    });
  }
}

export const db = new PMChronicleDB();
```

### 2.2 localStorage 使用項目

**キー**: `pm_chronicle_settings`

```typescript
interface Settings {
  volume: {
    bgm: number;       // 0-100
    sfx: number;
  };
  graphics: {
    quality: 'low' | 'medium' | 'high';
    animations: boolean;
  };
  gameplay: {
    autoSave: boolean;
    confirmActions: boolean;
    hintsEnabled: boolean;
  };
  lastPlayedSlot: number | null;
}
```

---

## 3. コアロジック詳細設計

### 3.1 ターン処理システム

**ファイル**: `src/lib/engine/turnProcessor.ts`

```typescript
export class TurnProcessor {
  
  /**
   * 1週間分のターン処理を実行
   * @param projectId プロジェクトID
   * @param policy 週の方針（NORMAL | QUALITY_FIRST | RUSH）
   * @returns 週次結果
   */
  async executeWeekTurn(
    projectId: string,
    policy: WeekPolicy
  ): Promise<WeekResult> {
    
    const project = await getProject(projectId);
    const assignedCharacters = await getAssignedCharacters(projectId);
    
    const dailyResults: DayResult[] = [];
    
    // 月曜: 初期化
    const monday = this.initializeWeek(project, policy);
    
    // 火～金: 日次シミュレーション
    for (let day = 1; day <= 4; day++) {
      const dayResult = await this.processDaySimulation({
        project,
        characters: assignedCharacters,
        policy,
        dayNumber: day
      });
      
      dailyResults.push(dayResult);
      
      // 重大イベント発生時は中断してユーザー介入待ち
      if (dayResult.criticalEvent) {
        return {
          status: 'PAUSED',
          pausedAt: day,
          criticalEvent: dayResult.criticalEvent,
          dailyResults
        };
      }
    }
    
    // 金曜: 週次集計
    const weekSummary = this.summarizeWeek(project, dailyResults);
    
    // EVM更新
    const updatedEVM = this.updateEVM(project, weekSummary);
    
    // スタミナ回復処理
    await this.recoverStamina(assignedCharacters);
    
    // オートセーブ
    await this.autoSave(project);
    
    return {
      status: 'COMPLETED',
      summary: weekSummary,
      evm: updatedEVM,
      dailyResults
    };
  }
  
  /**
   * 日次シミュレーション
   */
  private async processDaySimulation(params: DayParams): Promise<DayResult> {
    const { project, characters, policy, dayNumber } = params;
    
    const events: GameEvent[] = [];
    
    for (const task of project.wbs) {
      if (!task.assigneeId) continue;
      
      const character = characters.find(c => c.id === task.assigneeId);
      if (!character) continue;
      
      // 進捗計算
      const progressDelta = this.calculateProgress({
        character,
        task,
        policy
      });
      
      task.progress = Math.min(100, task.progress + progressDelta);
      
      // スタミナ消費
      const staminaCost = this.calculateStaminaCost({
        character,
        policy,
        serviceBonus: this.getTeamServiceBonus(characters)
      });
      
      character.stamina.current -= staminaCost;
      
      // トラブル判定
      const troubleEvent = this.checkTrouble(task, character);
      if (troubleEvent) {
        // 自動解決可能か判定
        if (this.canAutoResolve(character, troubleEvent)) {
          const resolved = this.autoResolve(character, troubleEvent);
          events.push(resolved);
        } else {
          // ユーザー介入が必要
          return {
            events,
            criticalEvent: troubleEvent
          };
        }
      }
      
      // ラッキーイベント判定
      const luckyEvent = this.checkLucky(character);
      if (luckyEvent) {
        events.push(luckyEvent);
      }
    }
    
    return { events };
  }
  
  /**
   * 進捗計算
   */
  private calculateProgress(params: ProgressParams): number {
    const { character, task, policy } = params;
    
    // 基礎進捗 = スキル値
    let baseProgress = this.getRelevantSkill(character, task.phase);
    
    // 方針補正
    const policyModifier = {
      NORMAL: 1.0,
      QUALITY_FIRST: 0.8,
      RUSH: 1.5
    }[policy];
    
    // 美貌ボーナス（周囲への影響）
    const charmBonus = character.statsRed.charm * 0.05;
    
    // 覚醒ボーナス
    const awakeningBonus = character.isAwakened ? 1.2 : 1.0;
    
    // 士気補正
    const moralModifier = character.mood / 100;
    
    const finalProgress = 
      baseProgress * 
      policyModifier * 
      (1 + charmBonus) * 
      awakeningBonus * 
      moralModifier;
    
    return Math.round(finalProgress);
  }
  
  /**
   * トラブル発生判定
   */
  private checkTrouble(task: Task, character: Character): GameEvent | null {
    // 品質スコアが低いほど発生率上昇
    const troubleRate = (100 - task.qualityScore) / 100;
    
    // 幸運で無効化
    const luckMitigation = character.statsRed.luck / 20; // 最大50%軽減
    
    const finalRate = troubleRate * (1 - luckMitigation);
    
    if (Math.random() < finalRate) {
      return {
        type: 'TROUBLE',
        severity: task.riskFactor > 70 ? 'critical' : 'warning',
        title: 'バグ発生',
        description: `タスク「${task.name}」でバグが検出されました`,
        taskId: task.id,
        impact: {
          progressRollback: -10,
          qualityPenalty: -5
        }
      };
    }
    
    return null;
  }
  
  /**
   * 自動解決判定（判断・直感スキル）
   */
  private canAutoResolve(character: Character, event: GameEvent): boolean {
    const judgmentThreshold = 7;
    return character.statsBlue.judgment >= judgmentThreshold;
  }
}
```

### 3.2 EVM計算ロジック

**ファイル**: `src/lib/engine/evmCalculator.ts`

```typescript
export class EVMCalculator {
  
  /**
   * EVM指標を計算
   */
  calculateEVM(project: Project): EVMState {
    const { wbs, budget, schedule } = project;
    
    // PV (Planned Value): 計画値
    // 現在週までに完了しているべき作業の予定価値
    const pv = this.calculatePlannedValue(wbs, schedule.currentWeek);
    
    // EV (Earned Value): 出来高
    // 実際に完了した作業の価値
    const ev = this.calculateEarnedValue(wbs);
    
    // AC (Actual Cost): 実コスト
    // 実際に消費した予算
    const ac = budget.initial - budget.current;
    
    // SPI (Schedule Performance Index): スケジュール効率指数
    // 1.0 = 予定通り、< 1.0 = 遅延、> 1.0 = 前倒し
    const spi = ev / pv;
    
    // CPI (Cost Performance Index): コスト効率指数
    // 1.0 = 予算通り、< 1.0 = 予算超過、> 1.0 = 予算内
    const cpi = ev / ac;
    
    // VAC (Variance at Completion): 完了時差異予測
    const vac = budget.initial - (ac + (budget.initial - ev) / cpi);
    
    return {
      pv,
      ev,
      ac,
      spi,
      cpi,
      vac,
      status: this.getEVMStatus(spi, cpi)
    };
  }
  
  /**
   * 計画値の算出
   */
  private calculatePlannedValue(wbs: Task[], currentWeek: number): number {
    return wbs.reduce((total, task) => {
      // タスクの予定完了週を計算
      const plannedCompletionWeek = this.getPlannedCompletionWeek(task);
      
      if (currentWeek >= plannedCompletionWeek) {
        // 完了しているべきタスク
        return total + task.estimatedEffort;
      } else {
        // 進行中タスクの予定進捗率
        const plannedProgress = this.getPlannedProgress(task, currentWeek);
        return total + (task.estimatedEffort * plannedProgress / 100);
      }
    }, 0);
  }
  
  /**
   * 出来高の算出
   */
  private calculateEarnedValue(wbs: Task[]): number {
    return wbs.reduce((total, task) => {
      return total + (task.estimatedEffort * task.progress / 100);
    }, 0);
  }
  
  /**
   * EVM状態評価
   */
  private getEVMStatus(spi: number, cpi: number): EVMStatus {
    if (spi >= 0.95 && cpi >= 0.95) {
      return 'HEALTHY';      // 緑: 健全
    } else if (spi >= 0.85 && cpi >= 0.85) {
      return 'WARNING';      // 黄: 注意
    } else {
      return 'CRITICAL';     // 赤: 危機
    }
  }
}
```

### 3.3 休暇調整パズルロジック

**ファイル**: `src/lib/engine/vacationPuzzle.ts`

```typescript
export class VacationPuzzleEngine {
  
  /**
   * 休暇調整パズルの生成
   * @param targetPeriod GW, 盆, 正月
   * @param projectRequirements 必要稼働日
   */
  generatePuzzle(
    targetPeriod: 'GW' | 'OBON' | 'NEWYEAR',
    projectRequirements: WorkRequirement[]
  ): VacationPuzzle {
    
    const characters = this.getProjectMembers();
    const calendarDays = this.generateCalendarDays(targetPeriod);
    
    // 各メンバーの希望休を生成
    const memberBlocks: MemberVacationBlock[] = characters.map(char => {
      return this.generateVacationPreference(char, calendarDays, targetPeriod);
    });
    
    return {
      calendarDays,
      memberBlocks,
      requirements: projectRequirements,
      budget: this.getAvailableBudget(),
      apPool: this.getAvailableAPPool()
    };
  }
  
  /**
   * メンバーの休暇希望を生成
   */
  private generateVacationPreference(
    character: Character,
    days: CalendarDay[],
    period: string
  ): MemberVacationBlock {
    
    const traits = character.traits;
    
    // 休暇重視型は長期間確保
    if (traits.includes('vacation_lover')) {
      return {
        characterId: character.id,
        requestedDays: this.getLongestBlock(days),
        isNegotiable: false,
        lockReason: '休暇取得は譲れません'
      };
    }
    
    // 計画重視型は早期確定済み
    if (traits.includes('plan_oriented')) {
      return {
        characterId: character.id,
        requestedDays: this.getRandomBlock(days, 5),
        isNegotiable: true,
        negotiationCost: {
          type: 'high_stress',
          description: '予定変更による大きなストレス'
        }
      };
    }
    
    // 報酬重視型は金銭で解決可能
    if (traits.includes('money_lover')) {
      return {
        characterId: character.id,
        requestedDays: this.getRandomBlock(days, 3),
        isNegotiable: true,
        negotiationCost: {
          type: 'budget',
          amount: 50000 * days.length
        }
      };
    }
    
    // 標準型
    return {
      characterId: character.id,
      requestedDays: this.getRandomBlock(days, 2),
      isNegotiable: true,
      negotiationCost: {
        type: 'ap',
        amount: 10
      }
    };
  }
  
  /**
   * パズルの解答可能性を検証
   */
  validateSolution(puzzle: VacationPuzzle, solution: VacationSchedule): ValidationResult {
    
    // 必須稼働日に誰も配置できていないかチェック
    for (const req of puzzle.requirements) {
      const availableMembers = this.getAvailableMembers(req.day, solution);
      
      if (availableMembers.length < req.minStaff) {
        return {
          valid: false,
          error: `${req.day}に${req.minStaff}名必要ですが、${availableMembers.length}名しか確保できていません`
        };
      }
    }
    
    // 予算チェック
    const totalCost = this.calculateTotalCost(solution);
    if (totalCost > puzzle.budget) {
      return {
        valid: false,
        error: `予算超過: ${totalCost.toLocaleString()}円（予算: ${puzzle.budget.toLocaleString()}円）`
      };
    }
    
    return { valid: true };
  }
}
```

---

## 4. UIコンポーネント詳細設計

### 4.1 ダブル・ヘキサゴンチャート

**ファイル**: `src/components/common/HexagonChart.tsx`

```typescript
import React from 'react';
import { ProjectSkills, TeamSkills } from '@/types/character';

interface HexagonChartProps {
  statsBlue: ProjectSkills;
  statsRed: TeamSkills;
  mode: 'blue' | 'red' | 'both';
  size?: number;
}

export const HexagonChart: React.FC<HexagonChartProps> = ({
  statsBlue,
  statsRed,
  mode,
  size = 200
}) => {
  
  const blueLabels = ['設計', '製造', '評価', '折衝', '提案', '判断'];
  const redLabels = ['庶務', '幹事', '給仕', '雑談', '美貌', '幸運'];
  
  const renderHexagon = (stats: number[], labels: string[], color: string) => {
    const points = calculateHexagonPoints(stats, size);
    
    return (
      <g>
        {/* 背景グリッド */}
        {[2, 4, 6, 8, 10].map(level => (
          <polygon
            key={level}
            points={calculateHexagonPoints(Array(6).fill(level), size).join(' ')}
            fill="none"
            stroke="#ccc"
            strokeWidth="1"
          />
        ))}
        
        {/* データポリゴン */}
        <polygon
          points={points.join(' ')}
          fill={color}
          fillOpacity="0.3"
          stroke={color}
          strokeWidth="2"
        />
        
        {/* ラベル */}
        {labels.map((label, i) => {
          const angle = (Math.PI / 3) * i - Math.PI / 2;
          const x = Math.cos(angle) * (size * 0.6);
          const y = Math.sin(angle) * (size * 0.6);
          
          return (
            <text
              key={i}
              x={x}
              y={y}
              textAnchor="middle"
              fontSize="12"
              fill="#333"
            >
              {label}
            </text>
          );
        })}
      </g>
    );
  };
  
  return (
    <svg width={size * 1.5} height={size * 1.5} viewBox={`-${size * 0.75} -${size * 0.75} ${size * 1.5} ${size * 1.5}`}>
      {(mode === 'blue' || mode === 'both') && 
        renderHexagon(Object.values(statsBlue), blueLabels, '#3b82f6')}
      {(mode === 'red' || mode === 'both') && 
        renderHexagon(Object.values(statsRed), redLabels, '#ef4444')}
    </svg>
  );
};

function calculateHexagonPoints(stats: number[], radius: number): string[] {
  return stats.map((value, i) => {
    const angle = (Math.PI / 3) * i - Math.PI / 2;
    const r = (value / 10) * radius * 0.5;
    const x = Math.cos(angle) * r;
    const y = Math.sin(angle) * r;
    return `${x},${y}`;
  });
}
```

### 4.2 EVMメーター

**ファイル**: `src/components/game/PMCockpit/EVMeter.tsx`

```typescript
import React from 'react';
import { EVMState } from '@/types/project';

interface EVMeterProps {
  evm: EVMState;
}

export const EVMeter: React.FC<EVMeterProps> = ({ evm }) => {
  
  const getColorBySPI = (spi: number): string => {
    if (spi >= 1.0) return '#22c55e';  // 緑
    if (spi >= 0.9) return '#eab308';  // 黄
    return '#ef4444';                   // 赤
  };
  
  const getNeedleRotation = (value: number): number => {
    // 0.5 ~ 1.5 の範囲を -90deg ~ +90deg にマッピング
    const clamped = Math.max(0.5, Math.min(1.5, value));
    return ((clamped - 0.5) / 1.0) * 180 - 90;
  };
  
  return (
    <div className="flex gap-4">
      {/* SPIメーター */}
      <div className="relative">
        <div className="text-xs text-center mb-1">SPI</div>
        <svg width="120" height="80" viewBox="0 0 120 80">
          {/* 円弧背景 */}
          <path
            d="M 20 60 A 40 40 0 0 1 100 60"
            fill="none"
            stroke="#e5e7eb"
            strokeWidth="8"
          />
          {/* 針 */}
          <line
            x1="60"
            y1="60"
            x2="60"
            y2="25"
            stroke={getColorBySPI(evm.spi)}
            strokeWidth="3"
            strokeLinecap="round"
            transform={`rotate(${getNeedleRotation(evm.spi)} 60 60)`}
          />
          {/* 中心点 */}
          <circle cx="60" cy="60" r="4" fill="#333" />
          {/* 数値表示 */}
          <text x="60" y="75" textAnchor="middle" fontSize="14" fontWeight="bold">
            {evm.spi.toFixed(2)}
          </text>
        </svg>
      </div>
      
      {/* CPIメーター */}
      <div className="relative">
        <div className="text-xs text-center mb-1">CPI</div>
        <svg width="120" height="80" viewBox="0 0 120 80">
          <path
            d="M 20 60 A 40 40 0 0 1 100 60"
            fill="none"
            stroke="#e5e7eb"
            strokeWidth="8"
          />
          <line
            x1="60"
            y1="60"
            x2="60"
            y2="25"
            stroke={getColorBySPI(evm.cpi)}
            strokeWidth="3"
            strokeLinecap="round"
            transform={`rotate(${getNeedleRotation(evm.cpi)} 60 60)`}
          />
          <circle cx="60" cy="60" r="4" fill="#333" />
          <text x="60" y="75" textAnchor="middle" fontSize="14" fontWeight="bold">
            {evm.cpi.toFixed(2)}
          </text>
        </svg>
      </div>
    </div>
  );
};
```

### 4.3 ガントチャート（簡易版）

**ファイル**: `src/components/game/PMCockpit/GanttChart.tsx`

```typescript
import React from 'react';
import { useDndContext, DragOverlay } from '@dnd-kit/core';
import { Task } from '@/types/task';
import { Character } from '@/types/character';

interface GanttChartProps {
  tasks: Task[];
  characters: Character[];
  currentWeek: number;
  onAssignChange: (taskId: string, characterId: string) => void;
}

export const GanttChart: React.FC<GanttChartProps> = ({
  tasks,
  characters,
  currentWeek,
  onAssignChange
}) => {
  
  const renderTaskBar = (task: Task) => {
    const assignee = characters.find(c => c.id === task.assigneeId);
    
    return (
      <div
        key={task.id}
        className={`relative h-8 mb-2 rounded ${
          task.isCriticalPath ? 'border-2 border-red-500' : ''
        }`}
        style={{
          width: `${task.progress}%`,
          backgroundColor: getPhaseColor(task.phase)
        }}
      >
        <div className="flex items-center justify-between px-2 h-full">
          <span className="text-xs text-white truncate">{task.name}</span>
          
          {/* リスク予兆アイコン */}
          {task.isRiskVisible && task.riskFactor > 70 && (
            <span className="text-red-600" title="高リスク検出">💀</span>
          )}
          
          {/* アサイン先 */}
          {assignee && (
            <div 
              className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-xs"
              draggable
              onDragStart={(e) => e.dataTransfer.setData('characterId', assignee.id)}
            >
              {assignee.name[0]}
            </div>
          )}
        </div>
      </div>
    );
  };
  
  return (
    <div className="p-4">
      <div className="text-sm font-bold mb-2">WBS進捗</div>
      {tasks.map(renderTaskBar)}
    </div>
  );
};

function getPhaseColor(phase: string): string {
  const colors = {
    REQUIREMENT: '#10b981',
    DESIGN: '#3b82f6',
    DEVELOP: '#8b5cf6',
    TEST: '#f59e0b',
    DEPLOY: '#ef4444'
  };
  return colors[phase] || '#6b7280';
}
```

---

## 5. AI連携詳細設計

### 5.1 直談判AI判定

**ファイル**: `src/lib/engine/aiJudge.ts`

```typescript
import { aiService } from '@/services/aiService';

export class AIJudge {
  
  /**
   * プレイヤーの自由記述を評価
   */
  async evaluateNegotiation(params: NegotiationParams): Promise<NegotiationResult> {
    const { situation, playerInput, opponent } = params;
    
    const prompt = this.buildEvaluationPrompt(situation, playerInput, opponent);
    
    const response = await aiService.analyze(prompt);
    
    // スコアリング（0-100）
    const score = this.parseScore(response);
    
    // 評価コメント抽出
    const feedback = this.parseFeedback(response);
    
    // 結果判定
    let result: 'GREAT' | 'GOOD' | 'NORMAL' | 'BAD';
    let effectMultiplier: number;
    
    if (score >= 80) {
      result = 'GREAT';
      effectMultiplier = 3.0;
    } else if (score >= 60) {
      result = 'GOOD';
      effectMultiplier = 2.0;
    } else if (score >= 40) {
      result = 'NORMAL';
      effectMultiplier = 1.0;
    } else {
      result = 'BAD';
      effectMultiplier = -0.5; // カウンターダメージ
    }
    
    return {
      score,
      result,
      effectMultiplier,
      feedback
    };
  }
  
  private buildEvaluationPrompt(
    situation: string,
    playerInput: string,
    opponent: OpponentProfile
  ): string {
    return `
あなたはプロジェクトマネジメントの専門家です。
以下の状況において、プレイヤーの説得文を3つの観点で評価してください。

【状況】
${situation}

【相手のプロフィール】
- タイプ: ${opponent.type}
- 重視する価値: ${opponent.values.join(', ')}

【プレイヤーの説得文】
${playerInput}

【評価観点】
1. 論理性（相手を納得させる根拠があるか）
2. 共感性（相手の立場を理解しているか）
3. 代替案（Win-Winの提案があるか）

各観点を0-10点で採点し、合計点と改善提案を出力してください。

出力形式（JSON）:
{
  "logic_score": 8,
  "empathy_score": 7,
  "solution_score": 9,
  "total_score": 80,
  "feedback": "論理的で代替案も優れています。さらに相手の懸念を明示的に言及すると完璧です。"
}
`;
  }
}
```

### 5.2 振り返りレポート生成

**ファイル**: `src/services/aiService.ts`

```typescript
export class AIService {
  
  /**
   * プロジェクト振り返りレポートを生成
   */
  async generateClosureReport(params: ReportParams): Promise<string> {
    const { project, logs, finalScore } = params;
    
    const prompt = `
あなたはベテランPMメンターです。
以下のプロジェクトログを分析し、新人PMへの振り返りレポートを作成してください。

【プロジェクト概要】
- 名称: ${project.name}
- 期間: ${project.schedule.duration}週
- 予算: ${project.budget.initial.toLocaleString()}円
- 最終スコア: Q=${finalScore.quality} C=${finalScore.cost} D=${finalScore.delivery}

【行動ログ】
${this.formatLogs(logs)}

【レポート構成】
# あなたのPMスタイル
（行動傾向の分析）

## 重要な決断
（成功/失敗の分岐点となった判断の解説）

## 学んだ用語
（ゲーム中に使用したPM用語の解説）

## KPT
- Keep（良かった点）
- Problem（課題）
- Try（次回試すこと）

マークダウン形式で、具体的かつ実践的なフィードバックをお願いします。
`;
    
    const response = await this.callLLMAPI(prompt);
    
    return response;
  }
  
  private formatLogs(logs: WeeklyLog[]): string {
    return logs.map(log => {
      return `Week ${log.week} [${log.eventType}]: ${log.description}${
        log.cardsUsed ? ` (使用カード: ${log.cardsUsed.join(', ')})` : ''
      }`;
    }).join('\n');
  }
}
```

---

## 6. 定数・マスターデータ

### 6.1 固有NPC定義

**ファイル**: `src/constants/npcDefinitions.ts`

```typescript
export const UNIQUE_NPCS: UniqueNPC[] = [
  {
    id: 'npc_the_mentor',
    name: '草壁 誠',
    ageGroup: '50s+',
    gender: 'male',
    jobClass: '元大手SIer PM',
    
    initialStatsBlue: {
      design: 8,
      develop: 5,
      test: 7,
      negotiation: 10,
      propose: 9,
      judgment: 10
    },
    
    initialStatsRed: {
      admin: 9,
      organizer: 8,
      service: 6,
      chat: 10,
      charm: 7,
      luck: 3
    },
    
    traits: ['veteran', 'mentor', 'slow_adopter'],
    
    hiddenData: {
      trueName: '伝説の火消し屋',
      description: '数々のデスマーチを生還させた伝説のPM',
      trueStatsBlue: {
        judgment: 10,  // 変化なし（既にMAX）
        negotiation: 10
      },
      secretTrait: 'miracle_worker',  // 炎上中のプロジェクトで真価発揮
      unlockCondition: {
        type: 'trust',
        threshold: 80,
        event: 'crisis_rescue'  // 炎上プロジェクトで彼の助言に従う
      }
    },
    
    personality: {
      description: '温厚だが、間違いは厳しく指摘する。理論より経験を重視。',
      negotiationStyle: 'logical_empathy',
      stressResponse: 'calm_problem_solver'
    }
  },
  
  {
    id: 'npc_genius_coder',
    name: '白石 リナ',
    ageGroup: '20s',
    gender: 'female',
    jobClass: '天才エンジニア',
    
    initialStatsBlue: {
      design: 9,
      develop: 10,
      test: 8,
      negotiation: 2,
      propose: 10,
      judgment: 4
    },
    
    initialStatsRed: {
      admin: 1,
      organizer: 2,
      service: 3,
      chat: 1,
      charm: 8,
      luck: 9
    },
    
    traits: ['genius', 'solo_player', 'innovation_lover'],
    
    hiddenData: {
      trueName: '孤高の開発者',
      description: '圧倒的な技術力だが、チームプレイが苦手',
      trueStatsBlue: {
        develop: 10,
        propose: 10
      },
      trueStatsRed: {
        luck: 9  // 天才ゆえの強運
      },
      secretTrait: 'code_artist',  // 単独タスクで200%効率
      unlockCondition: {
        type: 'achievement',
        description: '彼女に単独で難易度の高いタスクをアサインし、完璧に完了させる'
      }
    },
    
    personality: {
      description: '他人に興味がない。技術的に正しいことを最優先。',
      negotiationStyle: 'logic_only',
      stressResponse: 'silent_resign'  // ストレスで突然辞める
    }
  },
  
  {
    id: 'npc_political_master',
    name: '黒川 大輔',
    ageGroup: '30-40s',
    gender: 'male',
    jobClass: '営業上がりPM',
    
    initialStatsBlue: {
      design: 3,
      develop: 2,
      test: 4,
      negotiation: 10,
      propose: 8,
      judgment: 7
    },
    
    initialStatsRed: {
      admin: 7,
      organizer: 10,
      service: 9,
      chat: 10,
      charm: 9,
      luck: 6
    },
    
    traits: ['social_master', 'politician', 'smooth_talker'],
    
    hiddenData: {
      trueName: '社内政治の達人',
      description: '技術は低いが、人脈とロビー活動で案件を成功させる',
      trueStatsRed: {
        chat: 10,
        charm: 10,
        organizer: 10
      },
      secretTrait: 'stakeholder_whisperer',  // ステークホルダー交渉で無敵
      unlockCondition: {
        type: 'event',
        description: '政治的に困難な局面で彼の交渉術を目撃する'
      }
    },
    
    personality: {
      description: '人当たりが良く、誰とでも仲良くなれる。裏では計算高い。',
      negotiationStyle: 'win_win_creator',
      stressResponse: 'networking'  // 飲み会で解消
    }
  }
];
```

### 6.2 カード定義

**ファイル**: `src/constants/cardDefinitions.ts`

```typescript
export const NEGOTIATION_CARDS: Card[] = [
  {
    id: 'scope_creep',
    name: 'スコープ・クリープ',
    type: 'DEFENSE',
    cost: 2,
    description: '「それは契約範囲外です」と明確に拒否する',
    effect: {
      type: 'BLOCK_ATTACK',
      value: 100,
      targetGauge: 'dissatisfaction'
    },
    pmTermExplanation: 'スコープ・クリープとは、プロジェクトの範囲が無秩序に拡大すること。変更管理プロセスを通さない追加要求は明確に断るべき。'
  },
  
  {
    id: 'tradeoff',
    name: 'トレードオフ提案',
    type: 'NEGOTIATION',
    cost: 3,
    description: '「納期を優先するなら、この機能は次回リリースにしましょう」',
    effect: {
      type: 'REDUCE_OPPONENT_ATTACK',
      value: 50,
      addSatisfaction: 30
    },
    pmTermExplanation: 'トレードオフとは、一方を得るために他方を犠牲にすること。PMはQCD（品質・コスト・納期）のバランスを取る判断が求められる。'
  },
  
  {
    id: 'escalation',
    name: 'エスカレーション',
    type: 'ATTACK',
    cost: 5,
    description: '上司または決裁権者に判断を仰ぐ',
    effect: {
      type: 'HEAVY_DAMAGE',
      value: 80,
      sideEffect: {
        playerReputation: -10,
        description: '自分の評価が少し下がる'
      }
    },
    pmTermExplanation: 'エスカレーションは、自分の権限を超える問題を上位者に上げること。多用すると「判断力がない」と見なされるリスクがある。'
  },
  
  {
    id: 'psychological_safety',
    name: '心理的安全性の確保',
    type: 'SUPPORT',
    cost: 3,
    description: 'チームメンバーの発言を肯定し、士気を回復させる',
    effect: {
      type: 'HEAL_TEAM',
      value: 40,
      additionalEffect: {
        type: 'MORALE_BOOST',
        duration: 2
      }
    },
    pmTermExplanation: '心理的安全性とは、メンバーが恐れずに意見を言える環境。Googleの研究で、高パフォーマンスチームの最重要要素と判明。'
  },
  
  {
    id: 'direct_negotiation',
    name: '直談判',
    type: 'SPECIAL',
    cost: 10,
    description: '自分の言葉で相手を説得する（AI判定）',
    effect: {
      type: 'VARIABLE',
      aiJudged: true,
      minEffect: -20,  // 失敗時カウンター
      maxEffect: 200   // 大成功時
    },
    pmTermExplanation: '定型の手法ではなく、状況に応じた独自の説得を試みる。PMの真価が問われる瞬間。'
  }
];
```

---

## 7. パフォーマンス最適化

### 7.1 IndexedDB アクセスパターン

```typescript
// Bad: 毎回DBアクセス
function updateCharacter(id: string, updates: Partial<Character>) {
  const char = await db.characters.get(id);
  await db.characters.put({ ...char, ...updates });
}

// Good: バッチ処理
function updateMultipleCharacters(updates: { id: string, data: Partial<Character> }[]) {
  await db.transaction('rw', db.characters, async () => {
    for (const { id, data } of updates) {
      const char = await db.characters.get(id);
      await db.characters.put({ ...char, ...data });
    }
  });
}
```

### 7.2 React最適化

```typescript
// useMemo でヘビーな計算をキャッシュ
const ganttData = useMemo(() => {
  return calculateGanttLayout(tasks, currentWeek);
}, [tasks, currentWeek]);

// useCallback でイベントハンドラを安定化
const handleAssignChange = useCallback((taskId: string, charId: string) => {
  updateAssignment(taskId, charId);
}, [updateAssignment]);
```

---

## 8. テスト戦略

### 8.1 ユニットテスト

**対象**: ビジネスロジック（`src/lib/`）

```typescript
// evmCalculator.test.ts
describe('EVMCalculator', () => {
  it('should calculate SPI correctly', () => {
    const calculator = new EVMCalculator();
    const project = createMockProject({
      pv: 100,
      ev: 80
    });
    
    const evm = calculator.calculateEVM(project);
    
    expect(evm.spi).toBe(0.8);
    expect(evm.status).toBe('WARNING');
  });
});
```

### 8.2 統合テスト

**対象**: ターン処理フロー

```typescript
describe('Turn Processing Integration', () => {
  it('should complete a full week turn', async () => {
    const processor = new TurnProcessor();
    const projectId = await setupTestProject();
    
    const result = await processor.executeWeekTurn(projectId, 'NORMAL');
    
    expect(result.status).toBe('COMPLETED');
    expect(result.summary.progressDelta).toBeGreaterThan(0);
  });
});
```

---

## 9. セキュリティ考慮事項

### 9.1 XSS対策

- ユーザー入力（直談判テキスト）は全てエスケープ
- `dangerouslySetInnerHTML` は使用しない

### 9.2 データ改ざん

- チート対策は実施しない（教育用のため）
- ただし、JSONインポート時にバージョンチェックとチェックサム検証は実施

```typescript
function validateImportData(json: string): ImportValidation {
  const data = JSON.parse(json);
  
  if (data.version !== CURRENT_VERSION) {
    return { valid: false, error: 'バージョン不一致' };
  }
  
  const calculatedChecksum = generateChecksum(data.save_data);
  if (calculatedChecksum !== data.checksum) {
    return { valid: false, error: 'データが破損している可能性があります' };
  }
  
  return { valid: true };
}
```

---

## 10. 今後の拡張設計

### 10.1 モジュール分離

将来的なマルチプレイ対応を見据え、以下を分離設計：

- ゲームロジック: Pure Functions（副作用なし）
- 状態管理: Zustand（容易にサーバー同期可能）
- UI: Presentational Components

### 10.2 プラグインシステム

カスタムシナリオ対応のため、以下をJSON定義可能に：

- イベント定義
- カード定義
- NPC定義

---

**Document End**
