...
#### タスク一覧
- [ ] **Day 1-2**: 企業生成システム
  - [ ] `src/lib/generators/companyGenerator.ts`
  - [ ] 企業名生成（プレフィックス+中央+サフィックス）
  - [ ] 企業属性（規模、特性、得意分野）ランダム生成

- [ ] **Day 3-4**: NPC生成システム
  - [ ] `src/lib/generators/npcGenerator.ts`
  - [ ] 日本人名生成（姓500種+名300種）
  - [ ] 800人NPC生成（能力値、性格、所属企業）
  - [ ] 初期関係性生成（同期、上司部下）

- [ ] **Day 5**: 時代・歴史イベントシステム
  - [ ] `src/lib/events/historicalEvents.ts`
  - [ ] 時代区分定義（1990-2030年）
  - [ ] 歴史イベント（ITバブル崩壊、世界金融危機等）定義
  - [ ] スキルトレンド変化ロジック

- [ ] **Day 6-7**: NPC行動シミュレーション
  - [ ] `src/lib/simulation/npcSimulator.ts`
  - [ ] 転職、退職、定年ロジック
  - [ ] 企業興亡（破産、M&A）ロジック
